<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Lowongan KP') }}</h2>
    </x-slot>
    ...
    <div class="py-10">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white border border-gray-200 rounded-2xl shadow-sm">
                ...
                <h3 class="text-xl font-semibold text-gray-900">Edit Lowongan</h3>
                <p class="text-sm text-gray-500">Formulir perubahan lowongan KP</p>
                </div>
                <div class="p-6 text-gray-900">
                    <form action="{{ route('admin.lowongan.update', $lowongan) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="md:col-span-2">
                                ...
                                <select name="instansi_id" id="instansi_id" class="w-full rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" required>
                                    ...
                                    @foreach($instansis as $instansi)
                                        <option value="{{ $instansi->id }}" @selected(old('instansi_id', $lowongan->instansi_id) == $instansi->id)>{{ $instansi->nama_instansi }}</option>
                                    @endforeach
                                </select>
                                @error('instansi_id')
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="kebutuhan_keahlian" id="kebutuhan_keahlian" rows="3"
                                          class="w-full rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">{{ old('kebutuhan_keahlian', $lowongan->kebutuhan_keahlian) }}</textarea>
                            ...
                            <div>
                                ...
                                <input type="number" name="jumlah_kuota" id="jumlah_kuota" min="1"
                                       class="w-full rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm"
                                       value="{{ old('jumlah_kuota', $lowongan->jumlah_kuota) }}" required>
                                @error('jumlah_kuota')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" name="tanggal_mulai" id="tanggal_mulai"
                                       class="w-full rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm"
                                       value="{{ old('tanggal_mulai', optional($lowongan->tanggal_mulai)->format('Y-m-d')) }}" required>
                                @error('tanggal_mulai')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" name="tanggal_selesai" id="tanggal_selesai"
                                       class="w-full rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm"
                                       value="{{ old('tanggal_selesai', optional($lowongan->tanggal_selesai)->format('Y-m-d')) }}" required>
                            ...
                            <div class="md:col-span-2">
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="status_aktif" value="1" {{ old('status_aktif', $lowongan->status_aktif) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
