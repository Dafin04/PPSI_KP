<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Bimbingan</h2>
    </x-slot>
        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-lg p-6">
                ...
                <div class="mb-4 rounded-md border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">
                    Minimal {{ $minimumBimbingan ?? 10 }} bimbingan harus disetujui sebelum seminar dan unggah laporan akhir.
                ...
                <form id="formBimbingan" method="POST" action="{{ route('mahasiswa.bimbingan.update', $bimbingan->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <select name="proposal_id" required
                            ...
                            @foreach($proposals as $proposal)
                                <option value="{{ $proposal->id }}" @selected($proposal->id == old('proposal_id', $bimbingan->proposal_id))>
                                    {{ $proposal->judul }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            <input type="time" name="waktu_bimbingan" id="waktu_bimbingan"
                                class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                                value="{{ old('waktu_bimbingan', $bimbingan->waktu_bimbingan) }}" required>
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="topik_bimbingan"
                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                            value="{{ old('topik_bimbingan', $bimbingan->topik_bimbingan) }}" placeholder="Masukkan topik bimbingan..." required>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="catatan" rows="3" required
                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                            placeholder="Tuliskan pertanyaan atau bahasan yang ingin dibimbing...">{{ old('catatan', $bimbingan->catatan) }}</textarea>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        @if($bimbingan->file_lampiran)
                            <p class="text-xs text-gray-500 mt-1">Lampiran saat ini: <a class="text-indigo-600 hover:underline" target="_blank" href="{{ asset('storage/'.$bimbingan->file_lampiran) }}">Lihat</a></p>
                        @endif
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            form.addEventListener('submit', function (e) {
                ...
                // Abaikan tanggal saat ini jika sama dengan tanggal bimbingan yang sedang diedit
                const currentDate = "{{ $bimbingan->tanggal_bimbingan }}";
                const datesToCheck = existingDates.filter(d => d !== currentDate);
                if (datesToCheck.includes(selectedDate)) {
                    e.preventDefault();
